package edu.scu.mooddriftbottlerebuild.service;

import com.baomidou.mybatisplus.extension.service.IService;
import edu.scu.mooddriftbottlerebuild.entity.ReplyEntity;
import edu.scu.mooddriftbottlerebuild.entity.UsersEntity;
import edu.scu.mooddriftbottlerebuild.utils.PageUtils;

import java.util.List;
import java.util.Map;

/**
 * 
 *
 * @author liguohua
 * @email 3537136394@qq.com
 * @date 2023-05-20 00:09:36
 */
public interface UsersService extends IService<UsersEntity> {

    PageUtils queryPage(Map<String, Object> params);

    Map<String, Integer> getUserRecord(String openid);

    void violation(String[] openid, Integer sessionId);

    void receiveReport(int sessionId);

    List<ReplyEntity> getReportedReplyList();

    boolean isRegister(String open_id);
}

